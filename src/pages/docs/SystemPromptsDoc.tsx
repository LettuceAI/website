...
export function SystemPromptsDoc() {
  return (
    ...
    >
      ...
      <p>
        A system prompt is the hidden instruction layer that tells the AI how to
        behave. It is where you shape things like tone, boundaries, speaking
        style, and core behavior.
      </p>

      <ul>
        <li>Use it to shape behavior</li>
        <li>Do not use it as long-term memory</li>
        <li>
          Do not panic if this page looks advanced, most users barely need to
          edit it
        </li>
      </ul>

      <Callout type="info" title="Who this page is for">
        If you only want normal chatting or roleplay, the default system prompt
        is usually enough. This page matters most if you want to customize how
        the AI behaves very precisely.
      </Callout>

      <DocHeading level={3}>Safe beginner rules</DocHeading>
      <ul>
        <li>Keep instructions short and direct</li>
        <li>Describe behavior, not backstory</li>
        <li>Put world facts in lorebooks, not in the system prompt</li>
        <li>Let memory handle evolving story details</li>
        <li>
          Only go deeper into templates and injection rules if the defaults are
          not enough
        </li>
      </ul>
      ...
      <p>
        LettuceAI stores prompt templates in a database and renders them at
        runtime.
      </p>
      ...
      <ul>
        ...
        <li>
          A <strong>render pass</strong> replaces placeholders like{" "}
          <code>{"{{char.name}}"}</code>, <code>{"{{scene}}"}</code>,{" "}
          <code>{"{{context_summary}}"}</code>, and{" "}
          <code>{"{{key_memories}}"}</code>.
        </li>
        <li>
          A <strong>request builder</strong> then turns the rendered entries
          into provider-specific chat messages.
        </li>
      </ul>
      ...
      <ol>
        ...
        <li>
          It walks the template entries in order and checks whether each entry
      ...
      <Callout
        type="warning"
        title="Fallback is a safety net, not ideal placement"
      >
        ...
        <tbody>
          <tr className="border-b border-border/10">
            ...
            <td className="py-2 px-4">
              A saved prompt configuration with content, entry order, and an
              optional condense flag.
            </td>
          </tr>
          <tr className="border-b border-border/10">
            <td className="py-2 px-4 font-medium">Entry</td>
            <td className="py-2 px-4">
              A single prompt block with its own role, position, depth,
              conditions, and optional image binding.
            </td>
          </tr>
          <tr>
            <td className="py-2 px-4 font-medium">Rendered output</td>
            <td className="py-2 px-4">
              The final runtime message list actually sent to the model after
              placeholder replacement and filtering.
            </td>
          </tr>
        </tbody>
      </table>
      ...
      <p>
        ...
        well: inject memory only when available, insert reminder entries into
        the chat stream, gate entries by conditions, and attach image payloads
        in special internal templates.
      </p>

      <DocHeading level={3}>What one entry can control</DocHeading>
      <ul>
        <li>
          <strong>Role</strong>: system, user, or assistant.
        </li>
        <li>
          <strong>Position</strong>: relative, in-chat, conditional, or
          interval.
        </li>
        <li>
          <strong>Depth</strong>: how far back from the end of the chat buffer
          an in-chat entry is placed.
        </li>
        <li>
          <strong>Conditions</strong>: whether the entry should appear at all in
          the current runtime context.
        </li>
        <li>
          <strong>Payload binding</strong>: optional image slots in scene and
      ...
      <table className="min-w-full text-sm my-6">
        ...
        <tbody>
          <tr className="border-b border-border/10">
            ...
            <td className="py-2 px-4">
              Becomes part of the prompt stack before chat history is appended.
            </td>
          </tr>
          <tr className="border-b border-border/10">
            <td className="py-2 px-4 font-medium">In Chat</td>
            <td className="py-2 px-4">
              Always splices into the assembled chat-message list.
            </td>
          </tr>
          <tr className="border-b border-border/10">
            <td className="py-2 px-4 font-medium">Conditional</td>
            <td className="py-2 px-4">
              Splices into chat only when the current turn count is at least the
              configured minimum.
            </td>
          </tr>
          <tr>
            ...
            <td className="py-2 px-4">
              Splices into chat only when the current turn count is a non-zero
      ...
      <Callout type="info" title="Condense changes everything">
        ...
        one final system entry before injection. That flattening step
        effectively removes in-chat placement, per-entry roles, and depth
      ...
      <ol>
        <li>
          The engine first assembles the current chat-history message list.
        </li>
        <li>
        ...
        <li>
          Entries are sorted by insertion point, then by original template
          order.
        </li>
        <li>
      ...
      <p>
        In practice, depth <code>0</code> means the entry sits at the very end
        of the assembled chat-history buffer. Depth <code>1</code> places it one
      ...
      <ul>
        ...
        <li>
          <strong>Interval</strong> entries insert only when the message count
          is greater than zero and divides evenly by the configured interval.
        </li>
        <li>
      ...
      <p>When an entry becomes an API message, its role is normalized.</p>
      ...
      <ul>
        ...
        <li>
          <code>user</code> stays <code>user</code>.
        </li>
        <li>
          <code>assistant</code> stays <code>assistant</code>.
        </li>
        <li>
      ...
      <ul>
        ...
        <li>
          If summary, lorebook, or memory placeholders are absent, the engine
          can append generic fallback entries after rendering.
        </li>
      </ul>
      ...
      <ul>
        <li>
          Bound image slots such as <code>{"{{image[character]}}"}</code>,{" "}
          <code>{"{{image[persona]}}"}</code>,{" "}
          <code>{"{{image[avatar]}}"}</code>, and{" "}
          <code>{"{{image[references]}}"}</code> are converted into image parts
        ...
        <li>
          If an image-bound entry has no usable images, it simply does not emit
      ...
      <ul>
        <li>
          ...
          aliases like <code>{"{{char}}"}</code>, <code>{"{{persona}}"}</code>,
          and <code>{"{{user}}"}</code>.
        </li>
        <li>
      ...
      <p>
        ...
        the default direct-chat template expects core placeholders like{" "}
        <code>{"{{scene}}"}</code>, <code>{"{{scene_direction}}"}</code>,{" "}
        <code>{"{{char.name}}"}</code>, <code>{"{{char.desc}}"}</code>,{" "}
        <code>{"{{context_summary}}"}</code>, and{" "}
        <code>{"{{key_memories}}"}</code>.
      </p>
      ...
      <p>
        ...
        custom template omits <code>{"{{context_summary}}"}</code>,{" "}
        <code>{"{{key_memories}}"}</code>, or <code>{"{{lorebook}}"}</code>, the
        engine can still append fallback entries so that memory and lore are
        less likely to vanish entirely.
      </p>
      ...
      <ul>
        <li>
          <code>all</code>: every nested condition must match.
        </li>
        <li>
          <code>any</code>: at least one nested condition must match.
        </li>
        <li>
          <code>not</code>: inverts another condition.
        </li>
      </ul>

      <DocHeading level={3}>How AND and OR work in the editor</DocHeading>
      <p>
        The editor uses human-friendly <strong>AND</strong> and{" "}
        <strong>OR</strong>
      ...
      <ul>
        <li>
          <strong>AND</strong> means every rule in that joined group must be
          true.
        </li>
        <li>
          <strong>OR</strong> means any rule in that joined group can be true.
        </li>
        <li>
      ...
      <p>
        A useful mental model is: the editor first builds AND-groups and
        OR-groups, then combines those groups into the final condition tree.
        Exclusions are handled separately and become a final{" "}
        <code>not(...)</code> layer.
      </p>

      <p>
        So if you build include rules like <em>scene generation on</em> AND
        <em>character reference text exists</em> AND{" "}
        <em>persona reference text exists</em>, then add an exclusion that says{" "}
        <em>persona reference images missing</em>, the explanation becomes:
      </p>
      ...
      <table className="min-w-full text-sm my-6">
        ...
        <tbody>
          ...
          <tr className="border-b border-border/10">
            ...
            <td className="py-2 px-4">
              Compresses processed conversation windows into one rolling
              summary.
            </td>
          </tr>
          <tr className="border-b border-border/10">
            ...
            <td className="py-2 px-4">
              Runs the memory-manager tool pass that creates, deletes, pins, or
              unpins memories.
            </td>
          </tr>
          <tr className="border-b border-border/10">
            ...
            <td className="py-2 px-4">
              Generates a suggested next message for the user.
            </td>
          </tr>
          <tr className="border-b border-border/10">
            <td className="py-2 px-4 font-medium">Group chat and group RP</td>
            <td className="py-2 px-4">
              Formats multi-character context and message rules for group
              sessions.
            </td>
          </tr>
          <tr className="border-b border-border/10">
            <td className="py-2 px-4 font-medium">
              Avatar and scene generation
            </td>
            <td className="py-2 px-4">
              Builds image-generation prompts, including optional
              reference-image payload entries.
            </td>
          </tr>
          <tr>
            <td className="py-2 px-4 font-medium">Design reference writer</td>
            <td className="py-2 px-4">
              Turns reference images into a reusable design brief for later
              image work.
            </td>
          </tr>
        </tbody>
...
Content rules`}</CodeBlock>
      ...
      <ul>
        ...
        <li>
          Keep lore in lorebooks and let keyword matching inject only relevant
          entries.
        </li>
        <li>
          Keep memory placeholders if you want dynamic memory to stay readable
          and controlled.
        </li>
        <li>
          Prefer modular entry edits over turning the whole template into one
          giant paragraph.
        </li>
        <li>
          Use preview often when you add conditions, interval entries, or custom
          sections.
        </li>
      </ul>
