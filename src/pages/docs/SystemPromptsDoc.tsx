...
export function SystemPromptsDoc() {
  return (
    ...
    >
      ...
      <p className="lead">
        System prompts control how the AI behaves. They define rules, tone, and
        priorities that stay active throughout a conversation.
      </p>

      <DocHeading level={2}>What is a system prompt?</DocHeading>
      <p>
        A system prompt is a set of instructions sent to the AI before any user
        messages. It tells the model <em>who it is</em>,{" "}
        <em>how it should respond</em>, and <em>what matters most</em>.
      </p>
      <p>
        In LettuceAI, system prompts are usually attached to characters and
        applied automatically when you start a chat.
      </p>
      ...
      <p>
        Long prompts do not automatically mean better results. Clear
        instructions usually work better than complex setups.
      </p>

      <DocHeading level={2}>Writing effective system prompts</DocHeading>
      <p>
        Good system prompts focus on behavior, not lore. They describe{" "}
        <em>how</em> the character acts, not every detail of their history.
      </p>
...
You never break the fourth wall.`}</CodeBlock>
      ...
      <DocHeading level={2}>Prompt templates and presets</DocHeading>
      <p>
        The System Prompts editor is entry-based, similar to the Lorebook
        editor. This makes it easier to build modular prompts, reorder sections,
        and reuse templates.
      </p>
      <ul>
        <li>Import and export prompt presets for sharing</li>
        <li>
          Supports common community formats (SillyTavern, Tavern, and more)
        </li>
        <li>Mobile: long-press and drag to reorder entries</li>
      </ul>
      <DocSplit
        imageSrc={images.prompts.entryList}
        imageAlt="Prompt entry list view"
      >
        <p>
          Prompts are organized into entries so you can enable, reorder, or edit
          sections without touching the entire template.
        </p>
      </DocSplit>
      <DocSplit
        imageSrc={images.prompts.editEntry}
        imageAlt="Editing a prompt entry"
        reverse
      >
        <p>
          Each entry has its own role and injection position, letting you fine
          tune where the content appears in the final prompt.
        </p>
      </DocSplit>

      <DocHeading level={2}>System prompts and memory</DocHeading>
      <p>
        System prompts define <em>rules</em>. Memory defines{" "}
        <em>what happened</em>.
      </p>
      <p>
        If something should always be true, put it in the system prompt. If
        something should be remembered because it happened in conversation, let
        memory handle it.
      </p>
      ...
      <p>
        LettuceAI does not use a single hard-coded prompt. Instead, it builds
        prompts dynamically using a structured template and a set of
        placeholders.
      </p>
      <p>
        This makes prompts easier to reason about, safer to modify, and
        compatible with memory, personas, and world information.
      </p>

      <DocHeading level={2}>Template variables (placeholders)</DocHeading>
      <p>
        Template variables are placeholders wrapped in double curly braces, such
        as <code>{"{{char.name}}"}</code> or <code>{"{{scene}}"}</code>.
      </p>
      <p>
        When a conversation starts, LettuceAI replaces these placeholders with
        real data from the character, persona, memory system, and current chat
        state.
      </p>
      <p>
        Some variables are marked as <strong>required</strong>. These must exist
        in protected templates so core systems like memory and context injection
      ...
      <p>
        Scenes define context, not personality. This is why LettuceAI allows
        multiple starting scenes for the same character.
      </p>
      ...
      <ul>
        ...
        <li>
          <code>{"{{persona.desc}}"}</code>: The user persona’s background or
          role
        </li>
      </ul>
      <p>
        Personas let the AI react differently depending on who you are, without
        rewriting the character prompt.
      </p>
      ...
      <p>
        Lore is treated as established truth. Characters are expected to behave
        consistently with it when relevant.
      </p>
      ...
      <p>
        These are injected automatically. You should not manually rewrite or
        replace them.
      </p>
    ...
    **Pacing & Style:**
    ...
    {{content_rules}}`}</CodeBlock>
      ...
      <DocHeading level={2}>
        Why the default prompt is structured this way
      </DocHeading>
      <ul>
        <li>
          <strong>Clear separation of concerns</strong>: character, user, world,
          and memory are kept in distinct sections
        </li>
        <li>
          <strong>Safe memory injection</strong>: summaries and memories are
          added without overwhelming the model
        </li>
        <li>
          <strong>Editability</strong>: you can adjust behavior without breaking
          structure
        </li>
        <li>
          <strong>Consistency</strong>: characters stay grounded across long
          conversations
        </li>
      </ul>

      <p>
        This structure is intentional. Flattening everything into one paragraph
        usually makes characters less stable over time.
      </p>

      <Callout type="info" title="Important">
        If you customize the default system prompt, keep required placeholders
        intact. Removing them may disable memory or break roleplay flow.
      </Callout>
    </motion.article>
  );
}
